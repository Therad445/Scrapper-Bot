...
public class OrmLinkRepository implements LinkRepository {
    ...
    private SubscriptionEntity getOrCreateSubscription(ChatEntity chat, LinkEntity link) {
    ...
    public void add(long chatId, String url, Set<String> tags, Set<String> filters) {
        ...
        ChatEntity chat =
                chatJpa.findById(chatId).orElseThrow(() -> new IllegalArgumentException("Chat not found: " + chatId));

        LinkEntity link = linkJpa.findByUrl(url).orElseGet(() -> createNewLink(url));

        SubscriptionEntity sub = getOrCreateSubscription(chat, link);

        for (String t : tags) {
            TagEntity tag = tagJpa.findByName(t).orElseGet(() -> {
                TagEntity nt = new TagEntity();
                nt.name(t);
                return tagJpa.save(nt);
            });
            boolean exists = sub.tags().stream().anyMatch(st -> st.tag().equals(tag));
            if (!exists) {
        ...
        for (String f : filters) {
            FilterEntity fe = filterJpa.findByName(f).orElseGet(() -> {
                FilterEntity nf = new FilterEntity();
                nf.name(f);
                return filterJpa.save(nf);
            });
            boolean exists = sub.filters().stream().anyMatch(sf -> sf.filter().equals(fe));
            if (!exists) {
    ...
    public void removeTag(long chat, long link, String tag) {
        chatJpa.findById(chat).ifPresent(c -> c.subscriptions().stream()
                .filter(s -> s.link().id() == link)
                .findFirst()
                .ifPresent(s -> s.tags().removeIf(st -> st.tag().name().equals(tag))));
    }
    ...
    public void removeFilter(long chat, long link, String filter) {
        chatJpa.findById(chat).ifPresent(c -> c.subscriptions().stream()
                .filter(s -> s.link().id() == link)
                .findFirst()
                .ifPresent(s -> s.filters().removeIf(sf -> sf.filter().name().equals(filter))));
    }
    ...
    public List<LinkInfo> findAllByChat(long chatId) {
        return chatJpa.findById(chatId)
                .map(c -> c.subscriptions().stream().map(this::toInfo).toList())
                .orElse(List.of());
    }
    ...
    private LinkInfo toInfo(SubscriptionEntity s) {
        return new LinkInfo(
                s.link().id(),
                s.link().url(),
                s.link().lastCheckedAt(),
                s.link().lastUpdatedAt(),
    ...
    private String getUrl(long linkId) {
        return linkJpa.findById(linkId).map(LinkEntity::url).orElseThrow();
    }
}
